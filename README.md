...
```
pkhulnasoft_ebpf_process.<kernel version>.o
pkhulnasoft_ebpf_socket.<kernel version>.o
rkhulnasoft_ebpf_process.<kernel version>.o
rkhulnasoft_ebpf_socket.<kernel version>.o
...
```
...
`/usr/libexec/khulnasoft/plugins.d/`, for the collector to be able to access them.
...
1. Create a Git tag like so:
   ...
   This will kick off a Github Action Workflow that will Rebuild the Khulnasoft
...
Khulnasoft is open-source software and we are always open for contributions that
you can give us.
...
- We have the variable KHULNASOFTSEL, that selects where the functions will be
