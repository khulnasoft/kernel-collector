#define KBUILD_MODNAME "xfs_khulnasoft"
...
static __always_inline int khulnasoft_xfs_entry()
{
    ...
    libkhulnasoft_update_global(&xfs_ctrl, KHULNASOFT_CONTROLLER_TEMP_TABLE_ADD, 1);

    return 0;
}

SEC("kprobe/xfs_file_read_iter")
int khulnasoft_xfs_file_read_iter(struct pt_regs *ctx) 
{
    return khulnasoft_xfs_entry();
}

SEC("kprobe/xfs_file_write_iter")
int khulnasoft_xfs_file_write_iter(struct pt_regs *ctx) 
{
    return khulnasoft_xfs_entry();
}

SEC("kprobe/xfs_file_open")
int khulnasoft_xfs_file_open(struct pt_regs *ctx) 
{
    return khulnasoft_xfs_entry();
}

SEC("kprobe/xfs_file_fsync")
int khulnasoft_xfs_file_fsync(struct pt_regs *ctx) 
{
    return khulnasoft_xfs_entry();
}
...
static void khulnasoft_xfs_store_bin(__u32 bin, __u32 selection)
{
    __u64 *fill, data;
    ...
    libkhulnasoft_update_global(&xfs_ctrl, KHULNASOFT_CONTROLLER_TEMP_TABLE_DEL, 1);
}

SEC("kretprobe/xfs_file_read_iter")
int khulnasoft_ret_xfs_file_read_iter(struct pt_regs *ctx)
{
    __u64 *fill, data;
    __u64 pid_tgid = bpf_get_current_pid_tgid();
    ...
    khulnasoft_xfs_store_bin(bin, KHULNASOFT_KEY_CALLS_READ);

    return 0;
}

SEC("kretprobe/xfs_file_write_iter")
int khulnasoft_ret_xfs_file_write_iter(struct pt_regs *ctx)
{
    __u64 *fill, data;
    __u64 pid_tgid = bpf_get_current_pid_tgid();
    __u32 bin, pid = (__u32)(pid_tgid >> 32);
    ...
    data /= 1000;
    bin = libkhulnasoft_select_idx(data, KHULNASOFT_FS_MAX_BINS_POS);
    khulnasoft_xfs_store_bin(bin, KHULNASOFT_KEY_CALLS_WRITE);

    return 0;
}

SEC("kretprobe/xfs_file_open")
int khulnasoft_ret_xfs_file_open(struct pt_regs *ctx)
{
    __u64 *fill, data;
    __u64 pid_tgid = bpf_get_current_pid_tgid();
    __u32 bin, pid = (__u32)(pid_tgid >> 32);
    ...
    data /= 1000;
    bin = libkhulnasoft_select_idx(data, KHULNASOFT_FS_MAX_BINS_POS);
    khulnasoft_xfs_store_bin(bin, KHULNASOFT_KEY_CALLS_OPEN);

    return 0;
}

SEC("kretprobe/xfs_file_fsync")
int khulnasoft_ret_xfs_file_fsync(struct pt_regs *ctx) 
{
    __u64 *fill, data;
    __u64 pid_tgid = bpf_get_current_pid_tgid();
    __u32 bin, pid = (__u32)(pid_tgid >> 32);
    ...
    data /= 1000;
    bin = libkhulnasoft_select_idx(data, KHULNASOFT_FS_MAX_BINS_POS);
    khulnasoft_xfs_store_bin(bin, KHULNASOFT_KEY_CALLS_SYNC);
