...
struct bpf_map_def SEC("maps") tbl_nv_socket = {
    ...
    .key_size = sizeof(khulnasoft_nv_idx_t),
    .value_size = sizeof(khulnasoft_nv_data_t),
...
static __always_inline void set_common_tcp_nv_data(khulnasoft_nv_data_t *data,
                                               struct sock *sk,
                                               __u16 family,
                                               int state,
...
static __always_inline void set_common_udp_nv_data(khulnasoft_nv_data_t *data,
                                                   struct sock *sk,
                                                   __u16 family,
                                                   KHULNASOFT_SOCKET_DIRECTION direction) {
    __u32 tgid = 0;
...
int trace_udp_recvmsg(struct pt_regs* ctx)
{
    ...
    khulnasoft_nv_idx_t idx = {};
    __u16 family = set_nv_idx_value(&idx, sk);
    KHULNASOFT_SOCKET_DIRECTION direction = KHULNASOFT_SOCKET_DIRECTION_NONE;
    khulnasoft_nv_data_t *val = (khulnasoft_nv_data_t *) bpf_map_lookup_elem(&tbl_nv_socket, &idx);
    if (val) {
    ...
    khulnasoft_nv_data_t data = { };
    direction = KHULNASOFT_SOCKET_DIRECTION_OUTBOUND | KHULNASOFT_SOCKET_DIRECTION_INBOUND;
