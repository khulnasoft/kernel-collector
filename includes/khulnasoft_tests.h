...
#ifndef _KHULNASOFT_TESTS_H_
#define _KHULNASOFT_TESTS_H_ 1

#define KHULNASOFT_BTF_FILE "/sys/kernel/btf/vmlinux"
...
#define KHULNASOFT_CORE_DEFAULT_ERROR "It was not possible to attach JIT code for your kernel, try another\n" \
                                    "method or use eBPF programs from ../kernel directory.\n "
#define KHULNASOFT_KALLSYMS "/proc/kallsyms"

// Use __always_inline instead inline to keep compatiblity with old kernels

static inline int khulnasoft_ebf_memlock_limit(void)
{
...
static inline struct btf *khulnasoft_parse_btf_file(const char *filename)
{
...
static inline const struct btf_type *khulnasoft_find_bpf_attach_type(struct btf *bf)
{
...
enum khulnasoft_modes {
    KHULNASOFT_MODE_TRAMPOLINE,
    KHULNASOFT_MODE_PROBE,
    KHULNASOFT_MODE_TRACEPOINT
};
...
{
    const struct btf_type *type = khulnasoft_find_bpf_attach_type(bf);
    if (!type)
        return -1;
...
{
    uint32_t key = KHULNASOFT_CONTROLLER_APPS_ENABLED;
    uint32_t value = 1;
...
static inline char *khulnasoft_update_name(char *search)
{
    char filename[FILENAME_MAX + 1];
    char data[128];
    char *ret = NULL;
    snprintf(filename, FILENAME_MAX, "%s", KHULNASOFT_KALLSYMS);
